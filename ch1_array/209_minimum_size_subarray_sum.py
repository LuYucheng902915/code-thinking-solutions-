...
# bisect 是 Python 的一个标准库（Standard Library），不需要用pip安装
# 只要环境中安装了Python，就可以通过 import bisect

# bisect 模块的核心功能是帮助你在已排序的序列（主要是列表）中，高效地执行二分查找（Binary Search），并维持其排序状态
# 有一个很长的、已排序的列表，现在需要频繁地向其中插入新元素，同时还要保持列表的有序状态
# 常规的低效做法：my_list.append(new_item) 然后再 my_list.sort()。append 很快（O(1)），但 sort() 很慢（O(n log n)）。每次都重排整个列表，开销巨大
# bisect 的高效做法：bisect 模块使用二分查找来定位插入点，这个过程非常快（O(log n)）。然后执行插入操作（O(n)）。对于频繁插入的场景，这远比“追加再排序”要高效
...
# 如果x大于a中所有元素，返回的是列表的长度len(a)
# 这个返回值非常有意义，因为它正好是新元素应该被插入的索引位置，相当于执行 list.append() 的效果
...
# 如果列表有很多元素在排序上与插入的元素相等，left把插入元素放在最左边，right把插入元素放在最右边
...
# insort_left/right对于Python内置类型组成的列表，插入位置不同，结果相同
# 这是因为Python内置类型的比较，比如元组，会从左到右逐个元素比较。如果第一个元素相同，就比较第二个，以此类推
...
# Python作为面向对象语言，可以自定义比较的规则，比如下面的例子，可以通过定义类的特殊方法（比如 __lt__，即小于 < 操作符）
# 这告诉Python,例如比较对象时，可以只比较特定的属性，忽略其他属性。这使得两个内容不完全相同的对象，在排序意义上“相等了”
# 此时在left还是right插入，结果就有了明显的区别，下面的例子形象说明了这一点
...
class Solution:
    def minSubArrayLen(self, target: int, nums: List[int]) -> int:
        ...
        # 同样不会导致类型错误
        # 上面的写法虽然实际逻辑不会返回float，但是静态类型检查会报错
...
class Solution1:
    def minSubArrayLen(self, target: int, nums: List[int]) -> int:
        ...
        for right in range(n):
            ...
            while current_sum >= target:  # 这里while是重点，不是if
...
# Binary Search with Prefix Sums, O(n log n) time complexity
...
class Solution2:
    def minSubArrayLen(self, target: int, nums: List[int]) -> int:
        ...
        for i in range(n):  # i代表从nums[i]开始的子数组
            s = target + prefix_sums[i]
            # s代表需要找到>=s的最小前缀和
            # 使用二分查找找到第一个大于等于s的前缀和的位置
            # bisect_left返回的是第一个大于等于s的元素的索引
            # 如果没有找到，则返回len(prefix_sums)，即prefix_sums的长度
            # 在前缀和数组中，prefix_sums[j] - prefix_sums[i] 这个计算结果，精确地对应了原数组 nums 中子数组 nums[i:j]
            # nums[i:j]包含了从索引 i 到 j-1 的所有元素，因此其长度为 j - i，而不是 j - i + 1 这一定要注意
            # 内层仍然可以通过循环的方法， j从i + 1到 n，当 prefix_sums[j] - prefix_sums[i] >= target时
            # 说明从 i 到 j - 1 的子数组满足条件,此时检查 j - i 是否为当前最小长度。但这样做内层最坏会是O(n)，所以整体时间复杂度为O(n^2)
            # 二分查找由于前缀数组递增，可以将内层的查找时间复杂度降为O(log n)，因此整体时间复杂度为O(n log n)，但是代码不那么容易理解
