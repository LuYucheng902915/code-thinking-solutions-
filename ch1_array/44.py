...
# 首先代码在处理上有一个逻辑问题，那就是for j in range(m):(line 54)
...
# 它不会立即执行计算，而是返回一个map 对象。这个对象是一个迭代器 (iterator)，它封装了“将 int 函数应用于 ['1', '2', '3'] 中每一个元素”的这个操作逻辑。
...
只有当这个迭代器被“消费”(例如,被for循环遍历,或被list()、tuple()等类型构造函数强制“物化”)时
...
# Python创建了一个accumulate迭代器对象，我们叫它it
...
# 迭代器是一次性的
# 迭代器对象内部有一个状态，它会记住下一次应该返回哪个元素。当你通过for循环、list()、next()等方式请求数据时，它会交出下一个元素，并把自己的状态指向再下一个。当它交出最后一个元素后，它的状态就变成了“已耗尽”
...
# 这个“一次性”的设计让迭代器变得异常强大和通用。它可以优雅地表示：
# 无限序列：比如一个可以无限产生素数的生成器。你不可能把所有素数都存在内存里。你只能一个一个地向后获取，这个过程无法“重置”
...
# sys.stdin是一个文件类对象，完全符合迭代器的行为。Python所有文件对象默认都是可迭代的
# 可以被for循环遍历，是一次性的，会被耗尽，是惰性的，不会一次性将输入全部读入内存
# 对象被称为迭代器必须满足：
...
# 合二为一的角色（以文件对象和sys.stdin为例）
# 它的身份：既是“可迭代对象”，又是“迭代器”
# 它的职责：
# 当被iter()函数调用时，它的__iter__()方法不会创建新对象，而是直接返回它自己 (return self)
# 当被next()函数调用时，它的__next__()方法会交出下一个元素
# 比喻：一个自动售货机
# 它本身是一个容器（可迭代对象）
# 你按一下“购买”按钮（调用iter()），它不会给你一个单独的取货装置，它自己就是那个装置
# 你再按一下“出货”按钮（调用next()），它就自己吐出一个商品
# Python迭代协议核心概念总结
# Python的 for 循环是建立在“迭代协议 (Iteration Protocol)”之上的，该协议明确了两个相辅相成的角色：可迭代对象 (Iterable) 和 迭代器 (Iterator)
# for 循环的底层工作机制如下：
